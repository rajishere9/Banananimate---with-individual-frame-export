...
const AnimationPlayer: React.FC<AnimationPlayerProps> = ({ assets, onRegenerate, onBack }) => {
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      if (hoveredFrameIndex !== null && displayFrames[hoveredFrameIndex]) {
        const frame = displayFrames[hoveredFrameIndex];
        const rectX = frame.x * scale + offsetX;
        const rectY = frame.y * scale + offsetY;
        const rectW = frame.width * scale;
        const rectH = frame.height * scale;
  ...
  }, [viewMode, spriteSheetImage, displayFrames, getImageDisplayDimensions, hoveredFrameIndex]);
 ...
 const handleExport = () => {
  ...
  const getFrameIndexFromEvent = (e: React.MouseEvent<HTMLDivElement>): number | null => {
    if (!containerRef.current) return null;
    
    const rect = containerRef.current.getBoundingClientRect();
    const { scale, x: offsetX, y: offsetY, width: imgWidth, height: imgHeight } = getImageDisplayDimensions();
    
    const mouseX = e.clientX - rect.left;
    const mouseY = e.clientY - rect.top;

    if (mouseX < offsetX || mouseX > offsetX + imgWidth || mouseY < offsetY || mouseY > offsetY + imgHeight) {
        return null;
    }
    ...
    for (let i = 0; i < displayFrames.length; i++) {
        ...
        if (mouseX >= frameRect.x && mouseX <= frameRect.x + frameRect.width &&
            mouseY >= frameRect.y && mouseY <= frameRect.y + frameRect.height) {
            return i;
        }
    }
    return null;
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
      if (viewMode !== 'spritesheet') return;
      const index = getFrameIndexFromEvent(e);
  ...
  const handleDownloadSelectedFrame = () => {
    if (selectedFrameIndex === null || !frames[selectedFrameIndex]) return;
    
    const frameImageSrc = frames[selectedFrameIndex].src;
    const a = document.createElement('a');
    a.href = frameImageSrc;
    a.download = `bananimation-frame-${selectedFrameIndex + 1}.png`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-4xl">
      <div 
        ref={containerRef} 
        className={`relative w-full max-w-lg aspect-square bg-black rounded-lg overflow-hidden shadow-2xl mb-4 flex items-center justify-center ${viewMode === 'spritesheet' ? 'cursor-pointer' : ''}`}
            ...
            <>
              ...
              {viewMode === 'spritesheet' && spriteSheetImage && (
                <>
                  ...
                  />
                  <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-4 text-center z-10 backdrop-blur-sm pointer-events-none">
                    <p className="text-sm text-gray-200 max-w-prose mx-auto">
                        This animation was created with just one call to the 🍌 Gemini model by asking it to create this sprite sheet.
                         <span className="block text-amber-300 mt-2">Click any frame to export it.</span>
                    </p>
                  </div>
                </>
              )}
              ...
              <div className="bg-gray-800 rounded-lg shadow-2xl p-6 w-full max-w-md flex flex-col items-center">
                  ...
                  <div className="flex items-center gap-4 w-full">
                      <button
                          ...
                          className="w-full bg-gray-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-gray-500 transition-colors duration-200 flex items-center justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-gray-500"
                      >
                          <XCircleIcon className="w-5 h-5 mr-2" />
                          Cancel
                      </button>
                      <button
                          onClick={handleDownloadSelectedFrame}
                          className="w-full bg-green-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-green-500 transition-colors duration-200 flex items-center justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-green-500"
                      >
